...
# 代码包根目录（submission_llm_core/），.env 仅从这里读取。
# 本提交包已脱敏：仓库内只提供 .env.example 模板，不含任何真实密钥。
# 使用者需自行复制 .env.example 为 .env 并填入自己的 API key 后才会加载。
ROOT_DIR = Path(__file__).resolve().parents[1]
ProgressCallback = Callable[[dict[str, Any]], None]


def _load_dotenv_simple(env_path: Path | None = None) -> None:
    """从代码包本地目录加载 .env 到 os.environ，不依赖 python-dotenv。

    脱敏说明：仅读取本代码包根目录（submission_llm_core/.env），绝不回溯到
    主项目根目录。若本地不存在 .env（例如评审仅拿到 .env.example），则不加载
    任何密钥，静默跳过——所有真实 key 通过环境变量注入。
    """
...
# 模块加载时自动导入代码包本地 .env（若存在），确保 CLI 独立运行时能读取环境变量。
_load_dotenv_simple()
